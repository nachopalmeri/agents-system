...
- Cuando `.agents/memory/lessons-global.md` creció más de 50 líneas sin poda.
...
0. `.agents/projects-index.md` — lista de proyectos activos.
1. `.agents/memory/lessons-global.md` — lecciones ya promovidas.
2. `.agents/memory/developer_growth.md` — crecimiento y preferencias.
3. `.agents/tasks/lessons.md` (de cada proyecto listado en projects-index.md) — lecciones de sesión.
4. `.agents/tasks/todo.md` (de cada proyecto activo) — estado de tareas.
5. Git log reciente (últimos 20 commits) — qué se hizo.

## FASE 2 — PATTERN

Para cada lección en `.agents/tasks/lessons.md`, verificar:

1. ¿Apareció el mismo patrón en otros proyectos? (buscar en `.agents/projects-index.md` para encontrar proyectos activos, luego leer `.agents/tasks/lessons.md` de cada uno)
2. ¿Ya existe en `.agents/memory/lessons-global.md`?
...
Identificar lecciones stale en `.agents/memory/lessons-global.md`:

1. Lecciones sin mención en ningún `.agents/tasks/lessons.md` en los últimos 30 días.
...
El reporte se guarda en `.agents/memory/dream-reports/[fecha].md` y se presenta al humano.
...
4. Actualizar métricas en `.agents/memory/dream-reports/[fecha].md`.
...
```text
Sesión:  Harness captura → `.agents/tasks/lessons.md`
Entre sesiones: Dreaming cura → `.agents/memory/lessons-global.md` + rules + GOTCHAS
